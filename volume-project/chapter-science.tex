...
mode) with a \ktadj{40} detector and a \SI{1.07}\MW{} beam.

The DUNE far detector (FD) will be built as four \ktadj{10} modules, which will
...
%coupled to the large FD mass will 
...
%been assumed:
\begin{itemize}
\item Year 1: \SI{10}\kt{} FD mass,  \SIadj{1.07}{MW} \SIadj{80}{\GeV} proton beam with $1.47 \times 10^{21}$ protons-on-target per year beam, and no ND\footnote{A detailed discussion of the systematic uncertainties assumed
 with and without ND constraints is in \volphys.};
%%%(assume \num{5}\% signal systematic)
\item Year 2: Addition of the second \ktadj{10} FD module, for a total FD mass of
  \SI{20}\kt;
\item Year 3: Addition of the third \ktadj{10} FD module, for a total FD mass of
  \SI{30}\kt; and first constraints from the preliminary ND data analysis;
%%%%  (assume \num{3}\% signal systematic)
\item Year 4: Addition of the fourth \ktadj{10} FD module, for a total FD mass of
  \SI{40}\kt;
\item Year 5: Inclusion of constraints from a full ND data analysis; 
%%%%(assume  \num{2}\% signal systematic)
 \item Year 7: Upgrade of beam power to \SI{2.14}\MW; %%%(again at \SI{80}\GeV)
\end{itemize}
It is assumed that the knowledge from the near detector can be retroactively applied to previous data sets,
such that each improvement in the knowledge of systematic uncertainties is applied to the full exposure up to that point.
...
%lower the exposure needed to reach this level of sensitivity from
%\SI{400}\ktMWyr{} to around \SI{230}\ktMWyr{} (\num{6.5} years of
%running in the example staging plan).
...
wide range of neutrino energies over the \SIadj{1300}{\km} baseline.
% In the
...
function of time for \deltacp=0. In both figures the staging scenario described above was assumed.
...
% milestones. The numbers are for normal hierarchy using the current best fit values of the known oscillation parameters. 
%The two columns
 %  on the right are for different beam design assumptions. 
