...
To estimate the physics capabilities at each stage of the DUNE
project, we assume the 40-kt DUNE far detector (FD) will be built in
10-kt modules, which will come online over the course of several
years.  The near detector (ND) will likely start operations after FD
operations are already ongoing.  Without any ND constraints,
systematic uncertainties will be larger in the early stages of the
experiment. Additionally, the beam power is expected to be upgraded
from $\sim$1~MW to $\sim$2~MW during the course of the experiment. For
a rough estimate of the sensitivity of DUNE as a function of real time
for the first 10~years of operation, we consider the following staging
plan:
\begin{itemize}
...
\includegraphics[width=0.49\textwidth]{volume-physics/figures/mh_exp_staging}
\label{fig:mhexec}
\end{cdrfigure}
...
8.5 years in the 80 GeV 1.07~MW reference beam design, 
...
power(MW) X time (years) required to measure $\delta_{\rm CP} = 0 $ with a
precision better than $10^\circ$ ranges from 290 to 450 kt-MW-years
...
\begin{cdrfigure}[CP violation sensitivity and $\delta_{\rm CP}$
  resolution as a function of exposure]{execsummaryCP}{The
    significance with which CP violation can be determined for 75\%
     of \deltacp values as a function of exposure in years using the proposed staging plan outlined in this Chapter (left). The expected 1$\sigma$ resolution for \deltacp as a
    function of exposure in kt-MW-years (right). The shaded region represents the
    range in sensitivity due to potential variations in the beam
    design. This plot assumes normal mass hierarchy.}
\includegraphics[width=0.49\textwidth]{volume-physics/figures/cpv75_exp_staging}
...
discovery potential as demonstrated in Table
~\ref{tab:execosctable}. Several such potential improvements are discussed
...
\begin{cdrtable}[Exposures needed to reach physics
  milestones]{lcc}{execosctable}{The exposure in mass (kt) X proton beam power
    (MW) X time (years) needed to reach certain oscillation physics
    ...
    on the left are for different beam design assumptions. }
Physics milestone & Exposure kt-MW-years & Exposure kt-MW-years\\
...
de-excitation.  About 3000 events would be expected in a \ktadj{40}
