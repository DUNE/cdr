...
used targets and horns is sized based on the size of the NuMI components.  Following the NuMI design, the decay pipe is helium-filled, while the target chase is air-filled and air-cooled.  WATER COOLED????
...
The more advanced focusing system, called the `optimized beam configuration' and discussed in \volphys and \vollbnf, utilizes horns that are longer and larger in diameter and that are spaced farther apart than the reference design, which would require a target chase approximately 9~m longer and 0.6~m wider 
than the reference design.  It cannot be ruled out that further optimization, or new designs not yet studied %that may be 
called for in the future to explore new questions, may require additional space beyond this.  Also, the larger horns will require larger 
...
the target, which affects the integrated beam exposure.  Potential design developments range from incremental (e.g., changing from the reference design rectangular cross section, water-cooled graphite target % as in the current reference design 
to a cylindrical 
...
the impact on the absorber of increasing the diameter). 
  ...
  comparisons of the tradeoffs between 
 % cost tradeoffs of the physics advantages from 
  investment in the decay pipe versus investment in 
other systems, e.g., %the size of the 
a larger target hall complex, more advance target-horn systems, or more far detector mass.  Ongoing studies will continue to be carried out jointly by LBNF and DUNE between CD-1 and CD-2 to determine the baseline decay pipe geometry.

\fixme{not finished; more from Jim Strait}
