...
completed cryostat.
\begin{cdrfigure}[35-t prototype cryostat, cutaway view]{35cutaway}{(left) Cutaway view of the 35-t prototype cryostat. (right) Interior
...
\begin{cdrfigure}[Gas argon purge and recirculation in 35-t cryostat]{35TPurge}{Progress of the gas argon purge as it removes impurities  from the 35-t. The shown quantities are measured by various gas analyzers. The first stage of the purification is a process called the ``Piston Purge''.  The second stage is ``Recirculation with Filtering.'' The gap between the two steps was due to troubleshooting a leak.}
...
\begin{cdrfigure}[LAr electron lifetimes, 35-t cryostat]{35TElectronLifetime}{LAr electron lifetmes as measured by
...
in August 2015 after TPC installation, purge, cooldown and LAr fill. \fixme{check}
...
\begin{cdrfigure}[35-t cryostat with TPC]{35TTPC}{(left) Rendering of the
...
(purple) and splits the volume into one large and one very small %two
...
of the TPC.  } \includegraphics[width=0.55\textwidth]{35TDetector.png}
  \includegraphics[width=0.35\textwidth]{35TTrial}
\end{cdrfigure}

The Phase-2 prototype incorporates many novel DUNE single-phase far
...
%
\begin{cdrtable}[35-t Prototype design elements]{lcl}{35TDesign}{35-t Design Elements}
...
\item{measurement of distortions due to space charge and field non-uniformity, and}
