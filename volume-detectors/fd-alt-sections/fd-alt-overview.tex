...
channels, two identical collection views, and the absence of inactive volume %dead material 
in the LAr volume. These advantages will be described in
this chapter, and are covered in detail in \anxlbnob. \fixme{right annex? ask Dario}

\begin{cdrtable}[Dual-phase (alternative) far detector performance parameters]{llll}{FD_req}{%Subset of 
...
Electron Lifetime & 3~ms &  $>3$~ms 35-t prototype  & $>5$~ms \\ \colhline %membrane cryostat
...
and lbno, or is there another that wasn't named?  ask Dario}
...
%
The studies \fixme{one or two? ask Dario} were focused on the underground
...
non-evacuated vessel~\cite{WA105_TDR}. \fixme{That's all? Buy and fill? ask Dario} The S/N ratio can exceed 100
...
than 50:1 for tracks up to 6~m from the anode; % the first 6~m of drift; 
it reaches 14:1 for MIP tracks %after 12~m of drift. 
that are 6 to 12~m from the anode.
% \fixme{Do you mean: S/N is greater than 50:1 for tracks that are up to 6~m long, but only 14:1 for tracks longer than 12~m? The wording makes it sound like different values for different points in the track which doesn't make sense to me.}
...
%\fixme{one electrode made from one tube?} along a rectangular path with round corners. 
...
cathode plane is segmented % gridded 
%\fixme{in the form of a grid? What is gridded? Is it not solid, or is it solid and transparent? (Anne)
%Steve says: I think they mean "segmented" in the form of a grid, in order to reduce the cpacitance and danger from high voltage breakdown. I think it is a transparent solid, with electrical segmentation. } 
...
The proposed dual-phase detector module %based on the dual-phase design,
optimally exploits the cryostat volume % inner space foreseen for the DUNE cryostats
(14(w)$\times$14.1(h)$\times$62(l)~m$^3$) with an active area \fixme{area of anode?} of
(12$\times$60~m$^2$) and a drift length of 12~m, corresponding \fixme{to an active mass of?}
...
length provides a 1-kt increase in the active mass \fixme{active not fiducial?}  at a moderate
additional cost.
...
by a vertical field cage made up of 60 rectangular tubular rings \fixme{round cross section, bent into
rectangles, so the rings are not round but rectilinear?}
(tube diameter 140~mm, vertical spacing 200~mm). \fixme{what is the vertical space for? I'm not getting a picture}
...
\fixme{This was already said.} The cathode plane (on the bottom) is made by a reinforced frame, to
...
\fixme{this is part of configuration?} 
