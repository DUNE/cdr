...
channels, two identical collection views, and the absence of dead material \fixme{dead material or space?}  in
the LAr volume. These advantages will be described in
this chapter, and are covered in detail in \anxlbnob. \fixme{right annex?}

\begin{cdrtable}[Dual-phase (alternative) far detector design parameters]{llll}{FD_req}{Subset of performance parameters specific to the alternative far detector design.}  
...
\fixme{Parameters table: design or performance parameters? Pick one!}

The alternative design is the %result 
culmination of 13 years of R\&D and %two programs , <---- 2 programs was confusing (anne)
\fixme{It said `two programs' then said `the LAGUNA-LBNO study'; are the two programs laguna
and lbno, or is there another that wasn't named? glacier?}
...
LAGUNA-LBNO defined \fixme{designed?} an innovative, optimized and cost-effective configuration for
a long-baseline experiment. \fixme{ incorporating technological developments  (why does this need to be stated?}. 
%
The studies \fixme{one or two?} were focused on the underground
implementation of a very large LAr detector (GLACIER) and
beyond the dual-phase readout concept, involved many technical aspects \fixme{aspects or advances?}
...
\fixme{glacier is the name of the detector associated with the LBNO project?}
...
configuration is similar to a single-phase LArTPC but the features \fixme{specify which features}  of
the dual-phase design enable very long drift paths and large detector
dimensions \fixme{at this point the reader has no idea how big this thing is or how long the drift
paths are; maybe add these parameters to the table?} while minimizing the number of readout channels.
...
\fixme{Add a little more description in the figure caption; include dimensions}
...
2D readout plane on the top of the \fixme{gas or liquid?} volume. Readout units,
...
is around 20-100, \fixme{Given this range, do you need the word `about'?}
which %allows to 
...
non-evacuated vessel~\cite{WA105_TDR}. \fixme{That's all? Buy and fill?} The S/N ratio can exceed 100
...
electron-lifetime conditions and a LEM gain of \fixme{only} 25, the S/N is larger
than 50:1 for the first 6~m of drift; it reaches 14:1 for MIP tracks
after 12~m of drift. \fixme{Do you mean: S/N is greater than 50:1 for tracks that are up to 6~m long, but only 14:1 for tracks longer than 12~m? The wording makes it sound like different values for different points in the track which doesn't make sense to me.}

The single \fixme{added `single'} drift field (E${\simeq}$0.5--1.0~kV/cm) inside the fully active
...
\fixme{one electrode made from one tube?} along a
...
cathode plane is gridded \fixme{in the form of a grid? What is gridded? Is it not solid, or is it solid and transparent?} and is transparent in order to allow scintillation light to pass through and be
detected by uniform arrays of photomultipliers mounted
at the bottom of the tank, 1~m below the cathode surface. \fixme{if the cathode is thick enough that you
need to specify `surface', is it 1 m below the top or bottom surface?}

The drift field is %closed 
bordered at the top by the anode, which consists of a
collection of %modules 
devices for the charge readout called Charge Readout Planes %%%%% Anne to here 5/13
...
\begin{cdrfigure}[DUNE CRP 3~m$\times$3~m units.]{CRP_unit1}
