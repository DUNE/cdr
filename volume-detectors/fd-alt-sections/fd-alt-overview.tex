...
design described in Chapter~\ref{ch:detectors-fd-ref}, however this alternative
 design is under consideration for %could be used for 
 one or more
...
this alternative design satisfies the performance % DUNE physics
...
channels, two identical \fixme{but in x and y?} collection views, and the absence of dead
...
LAr detector (GLACIER) and involved \fixme{produced?} many technical advances with respect to
the tank, field cage and cathode, as well as advances in the areas of %logistics of the
...
\fixme{check minor edits in next 2 pgraphs (anne)}
...
\fixme{`segmented' raises questions; can we leave it off here?}
%
...
very long drift paths, large detector dimensions and far fewer %, while minimizing the number of 
...
\fixme{the above is edited version of following pgraph}
...
strips. \fixme{this is confusing because you don't know about the anode yet.}
...
has a longer drift length than the single-phase, %it should be noted that
 it requires a similar LAr
purity, % similar to that for the single-phase TPC, 
around 0.1~ppb (or
100~ppt) of oxygen equivalent. %corresponding to 
...
rectangles made of stainless-steel tubes %connected along a rectangular path 
with rounded corners.  \fixme{check edit prev sentence}
...
bottom of the cryostat. %The cathode plane 
It is a
...
system allows adjustment of the CRP distance and maintains the CRPs parallel to the LAr surface. \fixme{check edit; I replaced ``parallelism with respect
...
Situated in the vapor phase on top of the LAr volume, a CRP provides
...
by at least one order of magnitude, % by the possibility of amplifying the charges with avalanches in the gas phase.  This high S/N 
significantly
...
the cryostat, providing a large homogeneous fiducial volume % for neutrino interactions
free of embedded passive materials (effectively increasing the detector size),
reducing the number of readout channels, %increasing the detector size, 
and ultimately lowering costs.  
The CRPs collect the charge in a projective way, 
with practically no dead region and read the signals out %In each CRP the charge is readout 
in two collection views, eliminating the need for %with no use of
induction views, which %is also an important asset in the case of
simplifies the reconstruction of complicated topologies. \fixme{check above edits. Anne}
%The charge produced in this volume is collected on the top
...
LAGUNA and LAGUNA-LBNO design studies.  The CERN WA105 demonstrator, %experiment 
described in Section~\ref{sec:proto-cern-double}, is intended to prototype %show 
...
%All the key subsystems of the WA105 LArTPC
...
The inner detector for the \ktadj{12.1} active mass is built as a single
...
The cathode plane %(on the bottom) 
is constructed of a reinforced frame, to
guarantee its planarity, and filled by a %tubular <----- I can't picture `tubular grid'
grid (not visible in
Figure~\ref{fig:DP_det2}) 
\fixme{`filled with a grid' is awkward; strung with a wire grid?}
that allows transmission of %the optical transparency for the
scintillation light through it to %towards 
...
Each CRP unit is independently suspended by three stainless steel
