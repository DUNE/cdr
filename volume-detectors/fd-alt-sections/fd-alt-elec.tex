...
\fixme{Start with what are the components, how does the system work, briefly. Needs a short pgraph}
...
number in it:} A DUNE far detector module of this design would have nnn channels.
...
The design was driven by both cost and by the particular use of the
electronics for the dual-phase readout, which implies larger signals
from the detector \fixme{relative to what?} and allows releasing some
requirements. Section~\ref{sec:detectors-fd-alt-chg-readout} describes
the charge readout system. \fixme{Why reference the charge readout (above) and 
why repeat its description here? I suggest removing remainder
of pgraph.} In the dual-phase design, the ionization
...
bonus.  \fixme{Do we need next sentence?} White Rabbit is also used in the LBNO design as an independent
...
implemented in micro-TCA and provides triggers from the
photomultipliers that are distributed to the DAQ via the White Rabbit
...
There are actually two produced versions of the dual-phase ASIC chips
...
\begin{cdrfigure}[Dual-phase cryogenic ASIC amplifiers]{FE_ASIC1} {Front-end 16 channels cryogenic ASIC amplifier with the double-slope gain implementation}
