...
reduction and performance improvement. \fixme{Add a sentence with the right
number in it (nnn):} A DUNE far detector module of this design would have nnn channels.
...
from the detector \fixme{relative to single-phase?}, effectively eliminating requirements on noise. % and allows releasing some requirements. 
...
it accessible, and maintain the digital electronics externally, at room temperature.
 This provides risk mitigation and significant flexibility. \fixme{Do we need a reference for $\mu$TCA?}
...
bonus.  \fixme{Do we need next sentence? I would say keep it only if you can add something like ``A similar use 
may be possible in DUNE.''} WR is also used in the LBNO design as an independent
...
%The other feature of this R\&D program is
...
\fixme{above 2 pgraphs are rewrite of following 1 pgraph %% old
In the framework of the R\&D related to LAGUNA-LBNO, since 2006,
...
that the cryogenic front-end electronics, although at a very short
distance from the detectors in the CRP, is perfectly accessible at any
time without opening the cryostat due to the use of chimneys (see Section~\ref
This is achieved by implementing
the front-end cards at the bottom of the ad-hoc designed chimneys
which have two FTs (a warm and a cold FT). The cold
FT isolates the cards from the inner volume of the
...
\fixme{above  pgraph is rewrite of following 1 pgraph %% old
...
bipolar signals. Since 2012 some specific versions for the dual-phase
were produced in order to match the dynamic range of the signals
coming from the two collection views of the anode PCB after LEM
amplification. In this scheme, each collection view is instrumented
with strips of 3.125~mm pitch and 3~m length (150~pF/m capacitance),
as foreseen in the WA105 experiment. With a 3.125~mm strips pitch,
simulations of electromagnetic showers predict that a single channel
may collect up to 40~MIP. The design of the dual-phase cryogenic ASIC
is currently based on a LEM minimal gain of 20 which corresponds to
1200~fC for this maximal signal of 40~MIP.} %%
...
\fixme{why call it `global', there's been discussion about using that for the whole experiment, far and near detectors}
...
network. This happens together with the trigger time-stamp signals,
...
using the WR standard, through a dedicated, deterministic
...
system. \fixme{what is a node of the system? A channel or a shelf?}
 Constraints imposed on the data transfer bandwidth point towards use of 
the 10GbE protocol. For the
clock distribution, dedicated lanes must be defined by the user. The $\mu$TCA
standard also offers so-called \textit{clock} lanes, which distribute all boards
within the crate \fixme{distribute clock signals to boards?} and which may be used for any type of signal.
...
reference. \fixme{no idea what a ``retained shelf reference'' is}
...
shelf: \fixme{I'm not sure I understand the preceding phrase}
...
third FPGA, which sends the data on \fixme{to? from? I don't understand `on'} the backplane.  This readout
...
the backplane. As discussed, the $\mu$TCA standard %foresees special lanes for clock transmission. 
...
transfers and accurate synchronization. %among its different elements. 
...
ensures that all the Ethernet frames sent are delivered %at least 
after no more than a fixed delay 
\fixme{edit ok?} (controlled latency). The order of the frames should be
...
The WR application in the $\mu$TCA standard is currently engineered %targeted  for the moment, 
to easily interconnect %different 
$\mu$TCA cards. % between them. 
The \fixme{standalone WR?} switch
can therefore be connected directly to its different nodes in the same
rack to facilitate %improve 
maintenance and limit the space occupied. %occupancy. 
The implementation scheme is based on the integration of a WR mezzanine board on the MCH
of each shelf. 
In the future %it may be very powerful to have 
a full integration of the WR
within the $\mu$TCA DAQ system would be very powerful. %For the moment, the implementation
%scheme is based on the integration of a WR mezzanine board on the MCH
%of each shelf. 
...
%, with clear interest  and collaboration by the companies producing the MCH and the
