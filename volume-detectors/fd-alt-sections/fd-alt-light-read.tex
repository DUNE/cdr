...
cathode surface). The mechanics for the PMTs' attachment %fixation 
has been
...
detection surface, % below the TPC cathode, can reach up to 1000 units. 
the number may be as high as 1000.
The large number of photosensors calls for an integrated
solution for the front-end electronics \fixme{integration of this light-readout FE system with what?}
...
an important handle for cost reduction. \fixme{Seems you'd want to state technical solution
before cost benefit.} The signal digitization is
...
LAr. The detection of the direct scintillation light (S1) that provides the absolute event time
 is the main
purpose of the electronics. \fixme{of the electronics or of the system as a whole?} The
...
the gaseous phase. \fixme{how? that's at the top of the cryostat?}
...
completely autonomous from \fixme{independent of?} the other PMT arrays. The threshold is
...
photomultiplier after it has been correctly calibrated. In %Regarding
the timestamping process, there are two TDC ramps working in phase \fixme{phased opposition?}
opposition in order to reduce %, this way, 
the dead time (when the ramp goes to zero) by selecting the other ramp, \fixme{clarify: that will be in its linear
state.} When an event occurs, the value of the correct ramp is
...
that can be correlated with the one \fixme{with the signal?} coming from the PARISROC for
better precision. A prototype of the light-readout AMC is also being
developed %realized by still 
...
also the shape of the signal and the long-component decay of the light. \fixme{Did I reword it accurately?}
...
With this light readout architecture and the White Rabbit time-distribution 
