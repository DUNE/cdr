...
defined across the liquid-gas interface. %with
The field is applied between a submersed extraction
grid (stainless steel wires tensioned in both $x$ and $y$
directions) and the bottom of %. Once in the gas, the charge is amplified by 
...
\fixme{above pgraph is edited version of the following}
...
sides of the anodes. The signals from the last anode in each %forming the
strip chain are brought to feedthoughs
mounted on the other side of the front-end electronics embedded inside
dedicated signal-feedthrough chimneys using 50-cm-long flat cables. \fixme{prev sentence
confusing. ``other side of FE'' we haven't talked yet about either side of FE}

\fixme{I switched order of these two pgraphs}
...
suspension feedthroughs. It has its own high voltage system and %its
...
active; the 0.5-mm edge gaps %every 50~cm 
do not interfere with the
charge collection in the %3.125-mm readout pitch of the 
...
%%%% from old configuration section
...
\fixme{comb teeth?}  (see Figure~\ref{fig:Wires_comb}) inserted
...
benefit of maintaining the liquid level flat. \fixme{I cannot figure out what this looks like or how
it works.}
...
%%%%%%%%%%%
\fixme{above is edited version of the following}
...
flat cables going to the signal feedthrough}
 ...
 \fixme{why use m instead of cm here? I replaced with cm for consistency}
...
\fixme{I don't understand why the pattern isn't a straight grid. It has little arms inbetween
the tracks going off to nowhere.}
