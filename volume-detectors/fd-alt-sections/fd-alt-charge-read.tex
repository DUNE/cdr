...
\begin{cdrfigure}[Double phase readout]{setup}{Illustration of the amplification region in a double phase LAr TPC. The simulated field lines in dark blue are an
 indication of those followed by the drifting charges (without diffusion).}
 \includegraphics[width=.6\textwidth]{double_phase_principle.pdf}  
\end{cdrfigure}

\begin{cdrtable}[Interstage distances and electric field settings of the double phase readout components.]{lp{2cm}p{2cm}l}{crp_dist}{Interstage distances and electric field settings of the double phase readout components.}  %The third argument (reads {cc}) can use c, l, r or p{some length} 
...
\subsubsection{The charge readout plane for the LBNO 20 kton detector}
...
\begin{cdrfigure}[Side and top views of the $4\times4$~m$^2$ CRP]{4_4CRP_FRONT}{Side and top views of the $4\times4$~m$^2$ LBNO CRP (units in mm).}
 \includegraphics[width=\textwidth]{4_4_CRP_top-side-view}  
\end{cdrfigure}

%\begin{cdrtable}[Components of the 4$\times$4 m$^2$ CRP.]{lp{2cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}}{crp_para}{Components of the 4$\times$4 m$^2$ LBNO CRP.} 
%Component & $50\times50$ cm$^2$ anode & $50\times50$ cm$^2$ LEM & Signal  & Suspension  &readout strip &number of \\
%& panels & panels & feed through & feed through &length (m)& channels\\ \toprowrule
%4$\times$4 m$^2$ CRP   & 64 & 64 & 8 & 3 & 2 & 5120\\
%\end{cdrtable}

\begin{cdrtable}[Numbers of components of the 4$\times$4 m$^2$ CRP]{lr}{crp_para}{Numbers of components of the 4$\times$4 m$^2$ LBNO CRP} 
...
\begin{cdrfigure}[The wire holder, wire soldering PCB and the tensioning  system.]{wire_holder}{The wire holder, wire soldering PCB and the tensioning system.}
\includegraphics[width=.8\textwidth]{wire_holder}  
\end{cdrfigure}
...
\begin{cdrfigure}[Pictures of the LEM and anode along with microscope views ]{LEM_anode}{Top: pictures of the LEM and anode along with microscope
...
\begin{cdrfigure}[The 2D anode]{anode_sch}{The 2D anode (left) and its schematic explaining the
       interconnections between both views (right). One view is filled
       in red and the other in white.}
     \includegraphics[scale=0.26]{anode_pcb} \hspace{0.2cm} \includegraphics[scale=0.25]{anode_sch}
\end{cdrfigure}


\begin{cdrfigure}[Charge deposition as function of track angle ]{anode_res}{Charge deposition measured on view 0 ( $\Delta Q0/\Delta s0$) as a function  of the track angle $\phi$ (left) and projection of the  $\Delta Q0/\Delta s0$ distribution in three $\phi$ intervals (middle). The right plot  shows the distribution of the difference between the total charge  collected on both views normalised to their sum}
...
\begin{cdrfigure}[LEM performance vs geometry]{LEM}{Performance of the LEMs with different geometry
      parameters. Left: effective gain vs. LEM electric field; right:
      the stabilisations of the effective gain over time.}
     \includegraphics[scale=0.385]{4}
    \includegraphics[scale=0.315]{3}
\end{cdrfigure}
