...
is also simulated with custom algorithms.
...
Although not a mature field, there has been considerable
progress in recent years. In particular, the data from the
ICARUS~\cite{REF1} and ArgoNEUT experiments~\cite{REF2}
have enabled the development of a variety of reconstruction techniques,
which have formed the basis for a range of precision
neutrino physics measurements.
With the advancement of both single-phase and dual-phase technologies,
and expansion of the experimental program to include MicroBooNE~\cite{REF3},
the 35-ton prototype and the CERN test experiments,
the reconstruction software has continued to grow in volume and sophistication,
supported by the powerful frameworks such as LArSoft and Qscan.

A fully automated chain of event reconstruction algorithms
will be developed for the DUNE Far Detector,
as illustrated by the block diagram in Figure~\ref{fig:fdrecoblockdiag}.
The first stage of reconstruction involves the processing of the
noisy ADC wire signals, and the creation of 2D `hits'. 
A series of pattern recognition algorithms are then used to group
the hits into 2D and 3D clusters representing individual
particle tracks and showers. These objects then provide the input
for a set of high-level algorithms, which reconstruct the vertex
...
the algorithms have not yet been fully optimised and remain under
heavy development.
...
shape of each pulse. The fitted pulses are called hits for later processing.
...
introduces an additional discrete ambiguity by connecting multiple wire
...
and consistent arrival times. For most events, the majority of hits are
associated with a single wire segment, and are trivially disambiguated.
The remaining hits are disambiguated by clustering together the trivial hits.
This procedure constitutes the first pattern recognition stage in the
...
tracks and showers produced by individual final-state particles in an event.
The reconstruction of 3D objects is accomplished either by forming 2D clusters
and associating them between views, or by first associating 2D hits between views
and then clustering the resulting 3D hits. The clustering of Liquid Argon TPC hits 
is a challenging task, due to the variety and complexity of event topologies.
The development of automated pattern recognition algorithms for Liquid Argon 
TPC detectors remains a relatively new field, but several 2D and 3D algorithms
have been implemented, using a range of techniques.

The PANDORA software development kit~\cite{Marshall:2013bda,Marshall:2012hh}
provides fully-automated pattern recognition for both single-phase and dual-phase
detectors. PANDORA provides a highly modular approach to pattern recognition,
in which events are reconstructed using a large chain of algorithms,
each designed to identify a specific topology. 
A series of 2D pattern recognition algorithms are first used to
cluster together nearby hits, making use of the event topology.
A 3D reconstruction stage then follows, which associates the 2D clusters
between views, identifies the best-matched tracks and showers,
and modifies the clustering to improve the 3D consistency of the event.
vertex-finding algorithms are then applied, and a neutrino event
is formed by connecting together the 3D particles.
...
\includegraphics[width=0.6\textwidth]{pandora_uboone_vertex_resolution.pdf}
\end{cdrfigure}
