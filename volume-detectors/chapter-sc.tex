...
\fixme{A special Annex document for data rates was updated on 4/23/2015. It covers most DUNE physics and
background processes but the information is still being examined and modified. It is the reference document on
which important paramaters of the DUNE data are based.
...
oscillation physics studies with bean neutrinos, since these data will constitute the bulk of what's committed
...
\begin{table}[htbp]
  ...
  \input{generated/annex-rate-zs-volume-summary}
...
supported by powerful shared frameworks such as LArSoft and Qscan.

%Probably need to say that DUNE will require reconstruction supernova bursts, 
%atmospheric neutrinos and proton decay, not just accelerator neutrinos
...
In the dual-phase detector, the raw waveforms also undergo an initial processing stage, 
...
The main parameters determined by the fits are the hit time and the hit charge integral.
The procedure has been successfully tested during several phases of R$\&$D and prototyping on small-scale double phase LAr-LEM-TPC setups \cite{Badertscher:2008rf,Badertscher:2012dq}.
Figure~\ref{fig:lbnoeventdisplay} shows example event displays of the reconstructed hits 
in both real and simulated data. 
A detailed description of the signal-processing and hit-finding procedures
implemented for the single-phase and dual-phase techologies is given in
Sections~\ref{annex:reconstruction-signal-proc} and \ref{annex:reconstruction-single-phase-hit-find}.

In the single-phase APA design, the wrapping of induction-plane wires
...
as described in Section~\ref{annex:reco-pid}.
 
%\fixme{Do we have any calorimetry plots?}

\subsubsection{Neutrino Event Reconstruction and Classification}

%%%% Be sure to cite the Warwick paper here:
%%%% J. J. Back et al, Eur. Phys. C 2013, 73, 2369.

Once all the final-state particles in an event have been reconstructed individually, 
the combined information is used to reconstruct and classify the overall neutrino event.
The identification of neutrino event types is based on a multivariate analysis~\cite{REF, WA105_TDR,LAGUNA-LBNO-deliv,LAGUNA-LBNO-EOI},
which constructs a number of characteristic topological and calorimetric variables,
based on the reconstructed final-state particles, and uses a Boosted Decision Tree 
algorithm to calculate signal and background likelihoods for particular event hypotheses.
The current performance of the full reconstruction of pattern recognition and event classification 
has been evaluated using $\nu_{e}$ and $\nu_{\mu}$ charged-current interactions with two-body final states,
simulated in the dual-phase Far Detector. The correct hypothesis is chosen for 
92\% (79\%) of $\nu_{\mu}$ ($\nu_{e}$) quasi-elastic interactions with a 
lepton and proton in the final state, and 79\% (71\%) of $\nu_{\mu}$ ($\nu_{e}$) 
resonance interactions with a lepton and charged pion in the final state~\cite{LAGUNA-LBNO-deliv}.
The neutrino energy is estimated kinematically for selected quasi-elastic events,
using a two-body approximation, or otherwise a calorimetric energy measurement is implemented.
...
Figure~\ref{fig:recoenergynue} shows the results of the energy reconstruction for $\nu_e$ CCQE and CC1$\pi^{+}$ events.
