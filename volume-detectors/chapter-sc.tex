...
and the photon detection systems. The simulation provides
a basis for detailed studies of detector design and performance, 
and also enables the development of automated event reconstruction algorithms.
...
Although not yet a mature field, there has been considerable progress
in recent years.
...
supported by powerful frameworks such as LArSoft and Qscan.

%Say that DUNE programme will require reconstruction of accelerator
%neutrinos, supernova bursts, atmospheric neutrinos and proton decay ...
...
Specific hit finding algorithms have been developed for Dual-phase LAr-TPC charge signals.
Raw waveforms are processed in order to assure effective noise reduction and subtraction of the baseline.
...
Figure~\ref{fig:lbnoeventdisplay} shows events display from reconstructed hits for both data and MC events.
Detailed description of the signal processing and hit finding for the dual-phase charge readout can be found in the Sections~\cite{annex:dual-phase-signal-proc} and \cite{annex:dual-phase-hit-find}.
...
shown by ICARUS to have a resolution as good as $\Delta p/p \approx 10\%$ 
...
In the current scheme, the first stage is an examination of clusters 
...
These 3D parameters, combined with calorimetric information, enable the measurement
of the total energy of the shower, as well discrimination between electrons
and converted photons based on the ionization energy in the initial part of
the shower. The kinematic reconstruction final-state neutral pions from their
$\pi^{0} \rightarrow \gamma\gamma$ decays can also be performed by 
...
The identity of a particle track which ranges out in the active
volume of the detector may be ascertained by analysing the ionization density 
$dE/dx$ as a function of residual range from the end point of the track, 
and comparing with predictions for different particle species.  
Various techniques, from $\chi^2$ comparisons to neural networks,
have been developed, and are described in Section~\ref{annex:reco-pid}~\cite{Acciarri:2013met}.
