...
incorporating both the %Liquid Argon 
TPC modules
...
\fixme{The annexes aren't compiled along with this volume so the section reference below doesn't work.}
...
detectors is an active area of research and development that has advanced significantly in recent years.
...
the algorithms are not yet fully optimized and are subject to
...
\includegraphics[width=0.9\textwidth]{fdrecoflowchart.pdf}
\end{cdrfigure}
...
as described in \fixme{refs won't work} Sections~\ref{annex:reconstruction-signal-proc} and \ref{annex:reconstruction-single-phase-hit-find}.
...
validated using data from the 35-t prototype and CERN test experiment. \fixme{single-phase test?}
...
shape of each pulse. These pulses are called hits for later processing. \fixme{called `hits for later processing' or
called `hits' and are processed later?}
...
ensuring effective noise reduction and subtraction of the baseline. \fixme{pedestal?}
Hits, defined as signals that are discriminated from the noise, are then identified and reconstructed.
The hit-finding algorithms extract multiple hit information \fixme{information from multiple hits? or multiple
pieces of info about a given hit?}
...
described in Section~\ref{annex:reco-disambiguation}, \fixme{ref prob, maybe use `cite' for all these, as you do below} is used to break the
...
\fixme{Can we say `photon detector' to use same terminology throughout cdr?}
...
estimate.
\fixme{This may be enough info, but it feels truncated; maybe something about what algorithm is used, or some concluding sentence?}
...
%A series of <--- this made the singular-plural aspect awkward
...
for this purpose, as described in Section~\ref{annex:reco-pid}.\fixme{ref}
