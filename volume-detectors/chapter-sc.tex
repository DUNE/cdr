...
which will result in considerable contribution from radiological backgrounds. For this readon, the data rate that needs
to be handled in the process of SNB search can be expected to be quite significant. These is supported by estimates
presented in ``\anxrates''.
...
Characteristic
time scale for such SNB data capture will be $\sim$10s. Given the large amount of
data arriving within this time period (see the Appendix) and practical limits on the bandwidth of
the connection between the RCE data processors and frontend computers in DAQ, local
storage attached to data processors will be necessary to record the data at the peak rate. The buffer
in the data processor will not have sufficient capacity due to design and cost considerations (it can effectively
buffer about 0.4s of streaming data which is likely enough for the trigger decision but not for the complete
supernova event capture).
Preliminary estimates indicate that a storage device such as a SSD (one or two per board) will have
speed sufficient for this purpose. With trigger properly tuned, the rate of ``write'' operation
in the SSD can be kept sufficiently low so as to ensure their longevity. Once captured in this manner,
the data can then be transmitted to the rest of the farm within the available bandwidth.
...
\item The intensity of the beam provided by LBNF will affect the data
  ...
  writing. \fixme{Are you referring to something other than the
    reference design? If so, which configuration? If not, no need to
    state this.}
...
%\textit{For the moment, potential use of lossless data compression algorithms
%such as Huffman coding is not included in any of the estimates provided,
%since more studies need to be done to determine feasibility and efficacy of such approach.}
