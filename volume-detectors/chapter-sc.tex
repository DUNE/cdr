...
out of the LAr TPC in real time and being dominated by radiological backgrounds. Another differentiating feature is that multiple SNB neutrinos are expected
to arrive and interact in the detector during the possible rare Supernova burst event within seconds, as opposed to a rare single vertex produced by a beam neutrino.
To support SNB physics, massive and noisy data will need to be processed ``on the fly'' using approaches and
...
these modules as there is no concrete information developed at this point to support this approach.
\textit{Estimates presented below corresponds to the ``full detector'', i.e. is effectively normalized to th 40kt mass}.

The intensity of the beam provided by LBNF will affect the data rates for a few detector systems in DUNE (cf. the Near Detector).
