...
In the following, each of these items will be considered to produce relevant data rates estimates.
% estimated data rates for these components are itemized and then aggregated.
...
\end{itemize}
\
\fixme{A good suggestion from Amir to add a comment on the dynamic range... Need more info... --mxp--}
...
Non-ZS maximum event size (corresponding to a snapshot of the complete TPC) can be \textit{calculated as a product of the following numbers:}
\begin{itemize}
\item Channel count
...
\fixme{Brett, are you getting a better estimate from Tom? --mxp--}
...
\fixme{Brett, what do I need to do? --mxp--}
\begin{table}[htbp]
\label{table:data-volume}
  \centering
...
\end{table}
\fixme{Can you put the generated table into cdrtable format? (brett)} %%%%%% to here 4/29 Anne
...
\end{itemize}
\
\fixme{currently under investigation, questions sent to Mark Convery and Norm Buchanan --mxp--}
...
There are uncertainties in anticipated quantities of all of these types of data. Table \ref{table:data-volume} contains
...
\fixme{Amir is right, as we are mostly guessing the MC data volume - but I don't have more guidance at this point.
We are already stating it's more or less a guess --mxp--}
...
Crucial parameters of the alternative design of the Far Detector (based on the dual-phase technology)
are listed in Chapter 2 of the ``Expected Data Rates'' Annex. Here's a brief summary of some of its characteristics:
...
Longer descriptions of the current status of thenear detector simulation and reconstruction are given
...
based on parameterized detector responses, which is based on the GENIE~\cite{GENIE} and the achieved NOMAD simulation and reconstruction performance,
and the second is a full GEANT4-based simulation, which is under development.  The fast Monte Carlo tool is based on work done for
...
and extracting results from the Near detector.  
...
Once the visible particles in an event have been reconstructed individually, 
