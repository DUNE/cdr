...
determination of $t_0$ for non-beam %related 
events. Detection of the
scintillation light may also be helpful in background rejection and
triggering on non-beam events.

\fixme{You don't say whether the prompt and later photons are detected differently
in the system}
...
provide supernova neutrino detection. \fixme{I think what we're saying here is that
the ref PD design will NOT detect light associated with SN neutrinos, and therefore
that it really doesn't meet the requirements?} Alternative photon detector designs
are under investigation that would improve light detection for these low-energy 
events and improve their momentum resolution through determination
of the event t$_0$. The photon system will provide the $t_0$ timing of
events relative to TPC timing with a resolution better than 1~$\mu$s
(providing position resolution along drift direction of a couple of mm). \fixme{Shouldn't we say
something about $t_0$ in general, since that's its real purpose, if I understand}
%%%%%%%%%%%%%%%%%%%%%%%
...
and 12 SiPMs, as shown in Figure~\ref{fig:PD_overview}~(a). \fixme{check edit} %To enable this, the
The reference design for mounting %the PDs into the APA frames 
calls for
ten PD modules, approximately 2.2-m
...
peaked around 430~nm, is re-emitted inside the bar and transported %. The light guide
%transports the wavelength-shifted light 
to 12 silicon
