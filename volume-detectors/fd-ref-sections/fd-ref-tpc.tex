...
\fixme{up front we need to clarify use of the word `plane.' 
APA and CPA made up of planes of wires, right? Then APAs are tiled together in 2x25 sheets to form APA planes (different use of word) that extend the height and length of the entire TPC within a cryostat; CPAs same except 4x25. If I get confirmation on this, I will fix many of my own fixmes and remove use of word ``wall'' further down.}
...
boards are mounted on one end of the wire  \fixme{APA?}
...
``wire frame'') \fixme{can we get rid of the term `wire frame'?} as shown in Figure~\ref{fig:tpc-wire-frame-xsect}.

\fixme{The APA wire wrapping scheme figure could use more detail in the caption. Why are the x-sec dimensions of
the stainless steel frame important;  are the lines above and below them (on the right) the wire bonding boards? Maybe refer to Table~\ref{wire-parameters} for the wires. The image comes before the explanations.}
...
possible to place readout electronics only at one short edge of a wire \fixme{APA frame? `wire frame' is confusing}
...
these double sided APAs can be utilized by adding another plane of cathode \fixme{same as CPA or not? if so, call it CPA, if not, be more descriptive}
...
\fixme{does `wire plane' = `APA frame'?  Aha, looks like `yes'}
...
the ground by a couple of posts.  \fixme{check edit; this isn't what the picture shows} There
...
boards. \fixme{at the edge of the board? what's an `edge board'?} In this way the entire layer of wire can be placed on the
frame. 
...
\fixme{CPA is assembly of indiv cathode planes, right? So to call the 4x25 set of CPAs a `plane' is confusing. } 

There are two walls of Cathode Plane Assemblies (CPAs) %cathode planes 
in each detector module.  Each wall is %plane is
tiled from a four-unit-high by 25-unit-wide array of CPAs. Figure~\ref{fig:tpc-cathode-model} shows the
building blocks of a cathode wall. %plane.  
\begin{cdrfigure}[Conceptual design of CPA wall components ] %the cathode plane assembly]
{tpc-cathode-model}{Conceptual design of the different CPA wall %cathode plane 
components near a corner of a cathode wall.  Two flavors of CPAs (outer 
and inner unit) are used to make up the entire cathode wall; %of a cathode plane, 
the wall is terminated at %both 
...
minimize the surface electric field \fixme{magenta in the figure?}.  Two sets of field-shaping end
...
In the TPC, each pair of facing cathode and anode rows \fixme{oh boy, rows or walls?} forms an
...
The two cathode planes \fixme{CPA walls?} are biased at $-$180~kV to provide the required
...
The power supplies for the TPC cathode planes \fixme{CPA walls?} must be able to provide
