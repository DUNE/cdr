...
The preliminary numerical detector performance parameters are
...
values were achieved by Long Bo\cite{Bromberg:2015uia}. Long Bo
benefited from the use of cold electronics as will the DUNE far
detector though it had much shorter wires.
...
ADC's, as a sufficiently large gain will be necessary to detect weak
signals at the CPA, while at the same time, strong signals near the
APAs should be recorded without saturation if possible.  The
...
is the signature for a supernova burst event.  A \MeVadj{5} electron
...
from a proton-decay event resulting in a 100~MeV kaon with high
efficiency.)
...
detectors.  For a preliminary understanding of achievable energy
...
using the cascade of deexcitation $\gamma$ rays (which Compton-scatter
in the detector), which should serve the dual purposes of rejecting
