...
ArgoNeuT, although these detectors %experiments 
...
%The preliminary numerical %detector 
...
%Included are the parameters, 
%previous detector performance, and projected performance with references to relevant studies.  
Notes:
...
excursions of the noise over the zero-suppression threshold %be 
compose a
vanishingly small fraction of all ADC samples, and that it preserve the detector's %while preserving the
...
other sources, %we would like 
sufficient contingency on the
signal-to-noise ratio is necessary to ensure that the detector meets the
programmatic physics requirements. \fixme{What are `programmatic physics' requirements? Should be one or the other (I vote `physics')}  A value of 10:1 was achieved by
...
used cold electronics, % as will the DUNE far detector 
although its wires were much shorter than those planned for DUNE.

The electron lifetime requirement %of 
is set at 3~ms %is placed in order
 to
...
%, as a sufficiently large gain will be necessary 
to detect weak
signals at the CPA, but small enough to record %while at the same time, 
strong signals near the APAs %should be recorded 
...
The primary vertex position resolution requirement is intended %placed in order
...
weakest is that of the neutrino beam direction, and \fixme{therefore?} the asymmetry in the
...
muon momentum. \fixme{prev sentence very hard to parse; clarify} 
The expected performance of $\pm$18\% on the muon
momentum is \fixme{was?} achieved by ICARUS for a sample of stopping muons, where
the momentum measured by multiple scattering can be \fixme{was?} compared against
...
can be simulated, but \fixme{this fraction?} must be calibrated in data in order to give
