...
fabrication, testing, delivery and installation of the components (as illustrated in Figure~\ref{fig:fddaqblock}). \fixme{Is this list of components accurate in this context?} The DAQ system is required to:
\begin{itemize}
...
\end{itemize}
\fixme{why do some say `have the capability' and others just say `collect' or whatever; is there a subtle difference
that's important?}
...
based on the ICECube design. \fixme{reference?} This
...
under study: \fixme{check edit; the sentence was a mouthful!}
The non-zero-suppressed ring buffers on the current
design of RCEs are
sufficient for 0.4~s of buffering \fixme{is the preceding phrase one possible way? 
It doesn't read that way}; the trigger farm can send
instructions in the trigger messages to manage storage of data in
these buffers during a supernova. \fixme{and the preceding doesn't sound like a second possibility. This pgraph
needs a little work.} Two possible extensions
...
potential supernova event candidate. \fixme{what does it mean to keep a channel around the time of a candidate? Please clarify.}
...
be collected on the data stream; \fixme{that we want to collect on the data stream or that 
are expected to be collected even though we don't want them? (I guess no. 2); please reword}
 the physics beam events, atmospheric
neutrinos and other candidate contained events \fixme{`Candidate contained
event' seems awkward. Can we say `fully contained'? Or can we assume
that any candidate event is contained and therefore can remove the word `contained'? } 
