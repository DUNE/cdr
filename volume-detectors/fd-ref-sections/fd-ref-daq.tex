...
{\bf LArTPC detector readout} The LArTPC signals are digitised in a
...
detected.\fixme{Continue with this}.
...
\draw [proline,|-|] (-4,1.3) -- node [below,font=\small] {\rm Form PD packets} (-3,1.3);
...
\draw [proline,-|] (-5,4.4) -- node [above,pos=0.2,font=\small] {\rm ZS \& packetize} (-3,4.4);
\draw [proline,-|] (-5,3.7) -- node [below,pos=0.3,font=\small] {\rm Non-ZS ring buffer} (-0.6,3.7);
\draw [proline,|-,dotted] (-6.3010,3.7) -- (-5,3.7);

\draw [transline,<-] (-1.5,7.95) .. controls (-1.5,5.4) .. node [below,sloped,pos=0.35,font=\scriptsize] {} (-3,4.4);  
\draw [transline,<-] (-1.5,7.95) .. controls (-1.5,2.3) .. node [below=-3pt,sloped,pos=0.18,font=\scriptsize] {\rm send to trigger} (-3,1.3);  
...
\todo{DAQ1} Add text about the later part of the sequence, adjust text to
  use the same terms as above and on diagram.

\todo{DAQ2} Add table of rates and describe it.

\todo{DAQ3} Add short paragraph of assumptions made in costing etc.
