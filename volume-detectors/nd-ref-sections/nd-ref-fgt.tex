...
\includegraphics[width=1.1\textwidth]{FGT_Overview}
\end{cdrfigure}
...
\item 
Magnetized detector capable of  separating  as well as 
 $\mu^+$ from $\mu^-$, as well as   $h^+$ from $h^-$, where $h$ is a charged hadron 
...
\item Excellent momentum ($<5\%$) and angular ($<2$ mrad) resolutions for $\mu^{\pm}$, $e^{\pm}$, 
...
\item Electron/positron identification through the use of transition radiation (TR) in the 
      entire tracking detector for low-energy and/or large angle tracks (e.g. $\gamma$ conversions)  

\item $\pi/K/p$ identification by dE/dx in the entire tracking detector 
...
\end{itemize}
The requirements listed above imply the use of a low density, $\rho \sim 0.1$ g/cm$^3$, high-resolution magnetic spectrometer. 
...
\includegraphics[width=1.1\textwidth]{PartID_dedx}
\end{cdrfigure}
...
radiation length $X_0\sim 6m$. The momentum measurement requires that tracks are detected in at least 
...
\includegraphics[width=0.75\textwidth]{STT_Detail}
\end{cdrfigure}
