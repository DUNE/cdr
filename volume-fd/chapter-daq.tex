...
\begin{editornote}
%----------V--------%
  \fixme {Can remove this text -- superseeded by the bit above} 
...
% 1st row
 ...
 \&
 \& \node[boxa] (rce) {RCE\\digitization\\processors};
 \&
...
% Early attempts
%\draw[data] (rce) -- (node cs:name=trigfe,anchor=west);
%\draw[data] (rce) -- ($ (node cs:name=trigfe,anchor=south west)!0.7!(node cs:name=trigfe,anchor=north west) $);
%\draw[data] (rce.east) -- ($ (trigfe.south west)+(0,1cm)$);
...
%\coordinate (rceSR) at ($ (rce.south west)!0.8!(rce.south east) $);  % South right edge of RCE 
%\coordinate (sspNR) at ($ (ssp.north west)!0.8!(ssp.north east) $);  % North right edge of RCE 
...
%\draw[data] (rce.east) -- ($ (rce.east)+(0.2cm,0) $) |- ($ (trigfe.south west)!0.7!(trigfe.north west) $); 
...
%\draw[data] (rce) -- (datafe);

%\draw[data] (pd) -- (fthru);
%\draw[data] (fthru) -- (ssp);
%\draw[data] (ssp) -- (trigfe);
%\draw[data] (ssp) -- (datafe);
...
\draw[data,dotted] (fthru.north) -- ($(fthru.north)+(0,2.5cm)$) node [left] {In cryostat} node [right] {Room temp.};
\draw[data,dotted] (fthru.south) -- ($(fthru.south)-(0,2.5cm)$);
...
The task of data transfer is facilitated by having an essentially
...
\begin{cdrtable}[DAQ subsystem component counts]{ll}{daq15_component_counts}
  {DAQ subsystem component counts for one 20-kton module/cryostat.
    ...
    54  & COB Modules equipped with 8 RCEs\cr \hline
    ...
    1   & Slow Controls compute node\cr \hline
\end{cdrtable}
