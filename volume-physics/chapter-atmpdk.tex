...
Partially Contained (PC) depending on whether the lepton, with the vertex placed at a random position inside the 
detector, would be compeletly inside the fiducial wolume. and tracking the lepton until its edges. The number of events expected 
...
\caption[Atmospheric-neutrino event rates]
        {
         ...
         after applying lepton identification. 
        }
\begin{center}
\begin{tabular}{$L^c} %$ 
\toprule
...
electron-type events (left) and muon-type events (right), as a function of neutrino
...
With an exposure of \SI{350}{\ktyr}, DUNE will be able to distinguish between 
octants at 3$\sigma$ significance, based on atmospheric data alone, 
for true values of $|\theta_{23}-\pi/4|$ larger than about 6 degrees, 
...
Atmospheric neutrinos provide an alternativ emethod to geophysics to explore the Earth's interior.
MSW resonance effects on low-energy atmospherc neutrinos allow to learn about electron density in the 
Earth's core and, combined with the average matter density, provide unique information on the chemical compisition.

In addition, atmospheric neutrinos provide a window into a range of new physics scenarios, and can place limits
