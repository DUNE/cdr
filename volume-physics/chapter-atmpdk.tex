...
% - p.4-72: statement that Fig.4.4 shows that 10-kt DUNE detector can exceed SK limit on p -> K+ nubar in two years is incorrect. From the figure, the necessary exsposure is more like 50 kt*yr
...
\item Far Detector Mass: The far-detector fiducial size multiplied by
  ...
  scope in much less than that.  For atmospheric neutrinos, a
...
\item Far Detector Particle ID: It is required that the separation
  ...
  much less than one event into the %proton decay (per M.Sorel, changed 12/7/15)
  $p\to K^+\overline{\nu}$ sample.  Unlike the
