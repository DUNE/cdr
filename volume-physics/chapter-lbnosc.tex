...
\fixme{`particle physics would be complete' doesn't seem right. How about `the Standard Model appears complete'}
 except
for %the issues associated with (Anne changed 5/9)
 \fixme {some questions (or are they inconsistencies? I don't like `issues') remaining} in the neutrino sector, such as its unexplained flavor pattern.  These issues \fixme{inconsistencies, unknowns, puzzles?} 
...
and search for CP violation in the lepton sector \fixme{call it either lepton sector or neutrino sector if you are referring to the same thing; `lepton sector' includes more than just neutrinos, I know} by
...
matrix, known as the \textit{PMNS mixing matrix}: \fixme{Anne added `mixing' here instead of below}
\begin{equation}
...
The absolute values of the entries of the PMNS \fixme{rmv: mixing} matrix, which
...
\label{eq:ckmmatrix}
\end{equation}
and compared \fixme{change to: `for comparison to'} to the entries of the PMNS matrix given in Equation~\ref{eq:pmnsmatrix}.
...
sensitivities that depend on the design of the neutrino beam, \fixme{does the range depend or do the sensitivities depend?}
...
some of \fixme{summarizes the important properties?} the properties of the GEANT4 beamline simulations used to
...
Fast MC and \fixme{that are used?} used as inputs to GLoBES.  Figure~\ref{fig:eff} shows the
...
Sensitivity \fixme{sensitivities are obtained?} to determination of the neutrino mass hierarchy and
...
applied. \fixme{``most prob data set given blah'' implies ``no statistical fluct''?}
...
\fixme{check `to' ok?} probability of determining the
...
\begin{cdrtable}[Required exposure for a CP-violation measurement]{lcc}{cpv_requiredexposure}{The minimum exposure required to determine CP violation with a significance of 3$\sigma$ for 75\% of \deltacp values or 5$\sigma$ for 50\% of \deltacp values for the CDR reference beam design and the optimized beam design.}
 ...
 \toprowrule
 3$\sigma$ for 75\% of \deltacp values & \SI{1325}~\ktMWyr{} & \SI{855}~\ktMWyr{} \\
...
low-energy tune of the NuMI beam~\cite{numitdr}, 
\fixme{fix ref} but with a larger
...
reference beam, \fixme{elsewhere `optimized beam design' To fix} is based on the NuMI focusing horn design but uses a
...
relative parameters\fixme{`relative' parameters? Relative to what?}  used for the optimization of the first focusing
...
Due to the very small masses and large mixing of neutrinos, their oscillations over a long distance
  ...
  \end{itemize}
  \fixme{Fix placement of figures}
...
\item shall fully cover the energy range of the first oscillation
  ...
  $\sim2.4$~GeV for a \kmadj{1300} baseline. It shall also provide a
...
\item shall be built to provide a realistic estimation and whenever
  ...
  measurements from 60 to \GeVadj{120} %GeV/c 
...
\item Muon Neutrino and Antineutrino Events: The FD shall be capable
  ...
  events in sufficient numbers within the fiducial volume of the
  detector and identify the primary muon particle emerging from the
  main event vertex.  The total energy of the charged current event
  shall be measured.
