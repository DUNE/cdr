...
The LAr TPC performance parameters that go into the GLoBES calculation are generated using a fast Monte Carlo (MC) simulation, described in detail in \cite{Adams:2013qkq}.  The fast MC combines the simulated flux, the GENIE neutrino interaction generator~\cite{Andreopoulos:2009rq}, and a parameterized detector response that is used to simulate the reconstructed energy and momentum of each final-state particle.  The simulated energy deposition of the particles in each interaction is then used to calculate reconstructed kinematic quantities (e.g., the neutrino energy) and classify the type of neutrino interaction, including backgrounds and misidentified interactions.  Events are classified as $\nu_e$ CC-like, $\nu_{\mu}$ CC-like, or NC-like.  To reduce the NC and $\nu_{\tau}$ CC backgrounds in the $\nu_e$ CC-like sample, an additional discriminant is formed using the transverse momentum and other variables as inputs to a k-Nearest-Neighbor (kNN) machine-learning algorithm.  Figures xx-xx show the reconstructed-to-true energy smearing matrices from the fast MC that are used as inputs to GLoBES.  Figures xx-xx show the efficiency for different event types used in GLoBES based on the fast MC.
...
\begin{cdrtable}[Oscillation parameter values and relative uncertainties]{lcc}{oscpar_nufit}{Central value and relative uncertainty of neutrino oscillation parameters from a global fit~\cite{Gonzalez-Garcia:2014bfa} to neutrino oscillation data, for normal hierarchy. The relative uncertainty is computed using 1/6 of the 3$\sigma$ allowed range from the fit.}
...
% \begin{cdrfigure}[Variation in MH sensitivity due to $\theta_{23}$]{mh_theta23}{The variation in the MH sensitivity due to different values of $\theta_{23}$ within the allowed range.  The nominal value of $\sin^2\theta_{23} = 0.45$ provides a significance of at least $\sqrt{\overline{\Delta\chi^{2}}} = 5$ for all values of \deltacp.  The significance decreases for all values of \deltacp as $\sin^2\theta_{23}$ gets smaller.}
%  \includegraphics[]{}
% \end{cdrfigure}

% \begin{cdrfigure}[Variation in MH sensitivity due to $\theta_{13}$]{mh_theta13}{The variation in the MH sensitivity due to different values of $\theta_{13}$ within the allowed range.  The nominal value of $\sin^22\theta_{13} = 0.085$ provides a significance of at least $\sqrt{\overline{\Delta\chi^{2}}} = 5$ for all values of \deltacp.}
%  \includegraphics[]{}
% \end{cdrfigure}

% \begin{cdrfigure}[Variation in MH sensitivity due to $\Delta m^{2}_{31}$]{mh_deltamsq}{The variation in the MH sensitivity due to different values of \dm{31} within the allowed range.  The nominal value of \dm{31} = $2.46\times 10^{-3}$~eV$^2$ provides a significance of at least $\sqrt{\overline{\Delta\chi^{2}}} = 5$ for all values of \deltacp.}
%  \includegraphics[]{}
% \end{cdrfigure}
...
% and the statistical power as a function of exposure
% for 3$\sigma$ and 5$\sigma$ sensitivity. 

% \begin{cdrfigure}[MH sensitivity including statistical fluctuations]{mhstats}{
%   The variation in the MH sensitivity due to statistical fluctuations for a MH measurement with a minimum significance of $\sqrt{\overline{\Delta\chi^{2}}} = 5$ for 100\% of the values of \deltacp. See Figure~\ref{fig:mh_exposure} for the possible range of exposures to achieve this level of significance.
%   {\bf Left:} The 
...
The 40-kt DUNE detector will be built in 10-kt modules, which will come online over the course of several years (described in \voldune).  Additionally, the beam power is expected to be upgraded. Figure~\ref{fig:mh_staging} shows the MH sensitivity as a function of time considering this very preliminary staging plan. \fixme{Add specifics of staging when the plot is added.}

% \begin{cdrfigure}[Mass hierarchy sensitivity over time]{mh_staging}{The MH sensitivity as a function of time, including expected increases in detector mass and beam power over time.  The beginning time is defined as the time when the first detector module records neutrinos from LBNF.}
%  \includegraphics[]{}
% \end{cdrfigure}
...
% Table~\ref{tab:mh_requiredexposure} lists the minimum exposure required to determining the MH at a significance of $\sqrt{\bar{\chi^2}} = 5$ for 100\% of \deltacp values for both the CDR reference beam design and the optimized beam design.
