...
%section. 
  ...
  \emph{Quasi-elastic processes (QE)}: This interaction channel is comprised of events where the target nucleon 
  is neither excited, nor fragmented, conditions which require low 4-momentum transfer to the hadronic system ($Q^{2}$).
  The $Q^{2}$ in these interactions is low enough that the exact nature of the of the nuclear initial state
  becomes important. Current implemetaions of nuclear initial state models are inadaquate and the error on the sole
  free parameters in the free nucleon cross section model, $M_{A}^{QE}$, has been expanded to absorb the differnces between
  simulations and the observed $\nu$-nucleus scattering data. Better models of the nuclear initial state have been developed
  and are in currently being implemented in GENIE and other generators. These new models will be compared with current 
  and future data from \minerva, T2K, and upcoming LAr TPC experiemts. Once the new models are available studies will
  determine if the spectral variations in FD analysis samples resulting from fluctions on $M_{A}^{QE}$ within current uncertainties
  are consistent with those produced by the introduction of the new models. Eventually the set of models that best agrees with data
  will be adopted into the DUNE simulations and the uncertainties assigned to these models will reflect the agreement with the data.
  ...
  cross section due to unconstrained form factors and their use as effective parameters to absorb nucelar model effect.
  The second is the disagreement in outgoing pion kinematics between simulations and data.
  Data from T2K, \minerva, upcoming LAr TPC experiemts, and the DUNE ND should provide good constraints for DUNE
  ...
  background via pion production is quite large. Data from upcoming LAr TPC experiemts should help to constrain
...
primary interaction vertex with the nuclear medium. 
  ...
  Data from neutrino interactions in LAr TPCs from upcoming LAr TPC experiemts will provide detailed information on nucleon production
